...
class ContextMenuCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.conn = mysql.connector.connect(host="localhost", user='root', password="Sec901006458*", database="new_test")
    ...
    async def store_context_menu(self, interaction: discord.Interaction, message: discord.Message) -> None:
        try:
            ...
            self.c.execute("INSERT INTO accounts (message, date, userid, username) VALUES (%s, %s, %s, %s)", (message.content, formatted_date, sender, message.author.name))
            self.conn.commit()

            await interaction.response.send_message(f"Message stored.", ephemeral=True)
    ...
    async def retrieve_context_menu(self, interaction: discord.Interaction, member: discord.User = None):
        ...
        try:    
            ...
            self.c.execute("SELECT message, date FROM accounts WHERE userid=%s AND username=%s", (sender_id, sender_name))
